...
describe "/admin_cms_pages/new.html.erb" do
  include Admin::CmsPagesHelper
  
  before(:each) do
    assigns[:cms_page] = stub_model(Admin::CmsPage,
      :new_record? => true,
      :name => "value for name",
  ...
  it "renders new cms_page form" do
    ...
    response.should have_tag("form[action=?][method=post]", admin_cms_pages_path) do
